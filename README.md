...
- полный путь для хранения данных о паролях пользователя, который будет сохранен в `.py_pass_conf/config/config.ini` (папка конфига создается в корне папки пользователя системы). 
