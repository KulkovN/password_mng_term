...
`Важно`:
1) Не рекомендуется к  использованию на платформах MicrosoftWindows (программа не тестировалась на данная ОС, возможны ошибки в процессах манипуляции путей)
...
- Приложение обеспечивает прозрачный механизм хранения данных пользователя в примитивном виде с возможностью чтения данных, непосредственно скриптами приложения; 
...
*как пример для вашего ~/.zshrc || ~/.bashrc:*
...
- полный путь для хранения данных о паролях пользователя, который будет сохранен в `.py_pass_conf/config/config.ini` (папка конфига создается в корне папки пользователя системы. Мастер-пароль хранится в хеше). 
