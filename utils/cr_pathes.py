...
def configur() -> float:
    ...
    if not Path(path_for_cnfg).exists():
        ...
        profile = input('придумайте название первого профиля: ')
        ...
        config.set('Config', 'Path', f'{raw_profile_path}_pwds.json.aes') # для шифрования
        config.set('Config', 'MasterPaswd', _hash(paswd))
    ...
    pts, check_pswd, profile = \
        config.get('Config', 'Path'), config.get('Config', 'MasterPaswd'), config.get('Profiles', profile)
